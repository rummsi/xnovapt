 ...
 * @version 0.01  2/Dez/2014 21:30:59
 */
    ...
    function show() {
        ...
        if (filter_input_array(INPUT_POST)) {
            ...
            if ($game_config['secu'] == 1) {
                echo $_SESSION['secu'];
                if (!filter_input(INPUT_POST, 'secu') || filter_input(INPUT_POST, 'secu') != $_SESSION['secu']) {
                    $errorlist .= $lang['error_secu'];
                    $errors++;
                }
            }

            $post['email'] = strip_tags(filter_input(INPUT_POST, 'email'));
            ...
            $ExistUser = doquery("SELECT `username` FROM {{table}} WHERE `username` = '" . Database::$dbHandle->real_escape_string(filter_input(INPUT_POST, 'character')) . "' LIMIT 1;", 'users', true);
            if ($ExistUser) {
                $errorlist .= $lang['error_userexist'];
                $errors++;
            }
            // Si l'on verifiait que l'adresse email n'existe pas encore ???
            $ExistMail = doquery("SELECT `email` FROM {{table}} WHERE `email` = '" . Database::$dbHandle->real_escape_string(filter_input(INPUT_POST, 'email')) . "' LIMIT 1;", 'users', true);
            if ($ExistMail) {
            ...
            if ($errors != 0) {
                ...
                $QryInsertUser .= "`sex` = '" . Database::$dbHandle->real_escape_string(filter_input(INPUT_POST, 'sex')) . "', ";
                ...
                $NewUser = doquery("SELECT `id` FROM {{table}} WHERE `username` = '" . Database::$dbHandle->real_escape_string(filter_input(INPUT_POST, 'character')) . "' LIMIT 1;", 'users', true);
                $iduser = $NewUser['id'];
